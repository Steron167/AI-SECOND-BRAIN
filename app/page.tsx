  ...
  async function send() {
    ...
    try {
      ...
      if(data.reply) {
        ...
        if(data.memory_context) setMemory(data.memory_context)
      } else {
        throw new Error("No reply")
      }
    } catch (e) {
      console.error(e)
      setChats([...newChats, { role: "ai", text: "Groq Error - check API key in Vercel env vars and redeploy." }])
    }
    setLoading(false)
  }
    ...
    <div className="min-h-screen bg-[#faf9f5] text-black flex">
      <div className="w-[320px] bg-black text-white p-5 hidden md:block">
        ...
        <div className="mb-6">
          ...
          <div className="bg-zinc-900 p-3 rounded text-xs space-y-1">
            {memory.short?.length? memory.short.map((m: any, i: number) => <div key={i}>• {m.text || m.role}</div>) : <div className="text-gray-500">Empty... start chatting</div>}
          </div>
        </div>
        <div className="mb-6">
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex-1 p-6 overflow-y-auto space-y-4 max-w-3xl mx-auto w-full">
          {chats.map((c, i) => (
            <div key={i} className={`p-4 rounded-2xl ${c.role === "user"? "bg-black text-white ml-12" : "bg-white border shadow-sm mr-12"}`}>{c.text}</div>
          ))}
