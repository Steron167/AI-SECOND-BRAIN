...
export default function Page() {
  ...
  async function trackResearch() {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ message: `Topic: ${topic} vs Competitor: ${competitor || "general"} - plain comparison`, memory }),
      })
        ...
        return (
          <div key={idx} className="mb-5 rounded-2xl bg-[#fff7ed] border-2 border-orange-200 p-5">
            <div className="flex gap-2 mb-2 items-center">
              <div className="w-7 h-7 rounded-full bg-orange-500 text-white flex items-center justify-center text-xs font-bold">T</div>
              <div className="text-[12px] font-extrabold tracking-widest text-orange-700">THOUGHT</div>
              <div className="ml-auto text-[10px] px-2 py-1 rounded-full bg-orange-500 text-white font-bold">Agent Reasoning</div>
            </div>
        ...
        return (
          <div key={idx} className="mb-5 rounded-2xl bg-[#eff6ff] border-2 border-blue-200 p-5">
            <div className="flex gap-2 mb-2 items-center">
              ...
              <div className="ml-auto text-[10px] px-2 py-1 rounded-full bg-blue-600 text-white font-bold">Tool Invocation</div>
            </div>
      ...
      if (l.includes("observation:")) {
        return (
          <div key={idx} className="mb-5 rounded-2xl bg-[#f0fdf4] border-2 border-green-200 p-5">
            <div className="flex gap-2 mb-2 items-center">
              <div className="w-7 h-7 rounded-full bg-green-600 text-white flex items-center justify-center text-xs font-bold">O</div>
              <div className="text-[12px] font-extrabold tracking-widest text-green-700">OBSERVATION</div>
              <div className="ml-auto text-[10px] px-2 py-1 rounded-full bg-green-600 text-white font-bold">Evidence Gathered</div>
            </div>
        ...
        return (
          <div key={idx} className="rounded-2xl bg-black text-white p-6 border-2 border-black">
            <div className="flex gap-2 mb-4 items-center">
              <div className="w-7 h-7 rounded-full bg-white text-black flex items-center justify-center text-xs font-bold">V</div>
              <div className="text-[12px] font-extrabold tracking-widest">FINAL ANSWER - MULTI AI AGENT - VERIFIED</div>
              <div className="ml-auto text-[10px] px-2.5 py-1 rounded-full bg-white text-black font-bold">Full Length</div>
            </div>
  ...
  const Node = ({ id, title, sub }: any) => (
    <div className={`rounded-[14px] p-3 border-2 flex items-center gap-3 ${activeNode === id? "bg-white text-black border-white shadow-lg scale-[1.02]" : "bg-[#1e1e1e] border-[#2c2c2c] text-white"}`}>
      ...
      <div className="flex-1">
        ...
        <div className={`text-[10px] mt-0.5 font-medium ${activeNode === id? "text-zinc-600" : "text-zinc-400"}`}>{sub}</div>
      </div>
    ...
    <div className="min-h-screen bg-[#f8f6f2] flex">
      <div className="w-[380px] hidden lg:flex flex-col bg-[#0b0b0b] text-white p-6 shrink-0 border-r border-zinc-900">
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col gap-1 items-end">
            ...
            <div className="text-[9px] px-2.5 py-1 rounded-full bg-[#00ff66] text-black font-extrabold">6 AGENTS ACTIVE</div>
          </div>
        </div>
        ...
        <div className="mt-7 space-y-2.5">
          ...
          <div className={`mt-3 p-3.5 rounded-xl text-center text-[12px] font-extrabold border-2 ${activeNode === "done"? "bg-[#00ff66] text-black border-[#00ff66]" : "bg-[#171717] border-[#2a2a2a] text-zinc-500"}`}>
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-xl p-3 text-center border-2 border-white">
              <div className="text-[9px] font-bold text-zinc-500">CONFIDENCE</div>
              <div className="text-[16px] font-extrabold text-black mt-1">{meta?.metrics?.confidence || 0.85}</div>
            </div>
            <div className="bg-[#1c1c1c] rounded-xl p-3 text-center border-2 border-[#2c2c2c]">
              <div className="text-[9px] font-bold text-zinc-400">RETRIES</div>
              <div className="text-[16px] font-extrabold text-orange-400 mt-1">{meta?.metrics?.retries || 0}</div>
            </div>
            <div className="bg-[#1c1c1c] rounded-xl p-3 text-center border-2 border-[#2c2c2c]">
              <div className="text-[9px] font-bold text-zinc-400">STEPS</div>
              <div className="text-[16px] font-extrabold text-white mt-1">{checkpoints.length || 6}</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <div className="bg-[#151515] rounded-xl p-3 border border-zinc-800 max-h-[200px] overflow-y-auto space-y-2">
            {chats.map((c, i) => (
              <div key={i} className={`text-[11px] p-2 rounded-lg ${c.role === "user"? "bg-white text-black font-bold" : "bg-zinc-800 text-zinc-300"}`}>
                <span className="font-extrabold">{c.role.toUpperCase()}: </span>{c.text.slice(0, 120)}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-auto pt-6 text-[10px] text-zinc-600">
          <div>Framework: LangGraph | Runtime: Next.js</div>
          <div>Tools: web_search (fallback) + vault_search</div>
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="p-6 md:p-8 max-w-[900px] mx-auto w-full">
          ...
          <p className="text-[14px] font-medium text-zinc-600 mt-1 flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-full bg-black text-white text-[11px] font-bold">MULTI AI AGENT</span>
            6 agents - Parallel - Full ReAct - Runtime on LangGraph
          </p>
            ...
            <div className="bg-[#f6f4f1] rounded-[18px] p-5">
              <div className="grid md:grid-cols-[1.3fr_1fr] gap-3">
                <div>
                  ...
                  <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="" className="w-full mt-1 bg-white rounded-full px-5 py-3.5 text-[15px] font-semibold text-black border-2 border-zinc-200 focus:border-black outline-none" />
                </div>
                <div>
                  <label className="text-[11px] font-bold text-zinc-500 ml-1 tracking-widest">VS COMPETITOR</label>
                  <input value={competitor} onChange={(e) => setCompetitor(e.target.value)} placeholder="" className="w-full mt-1 bg-white rounded-full px-5 py-3.5 text-[15px] font-semibold text-black border-2 border-zinc-200 focus:border-black outline-none" />
                </div>
              </div>
              <button onClick={trackResearch} disabled={loading} className="w-full mt-4 bg-black text-white py-4 rounded-full font-extrabold text-[15px]">
                {loading? "MULTI AGENTS RUNNING..." : "Run Research → 6 Agents → ReAct"}
              </button>
              {meta && (
                <div className="mt-3 flex gap-2 text-[10px] font-bold">
                  <span className="px-2.5 py-1 rounded-full bg-black text-white">LangGraph: {checkpoints.length} checkpoints</span>
                  <span className="px-2.5 py-1 rounded-full bg-[#00ff66] text-black">Confidence: {meta?.metrics?.confidence}</span>
                  <span className="px-2.5 py-1 rounded-full bg-white border">Parallel: yes</span>
                </div>
              )}
            </div>
          </div>
          ...
          {result && (
            <div className="mt-6 bg-white rounded-[24px] border-2 border-black shadow-xl overflow-hidden">
              <div className="px-6 py-4 bg-black text-white flex justify-between items-center">
                <div className="text-[11px] font-extrabold tracking-widest">FINAL ANSWER • MULTI AI AGENT • FULL LENGTH</div>
                <div className="flex gap-2">
                  <span className="text-[11px] px-3 py-1 rounded-full bg-white text-black font-bold">{result.length} chars</span>
                  <span className="text-[11px] px-3 py-1 rounded-full bg-[#00ff66] text-black font-bold">6 Agents</span>
                </div>
              </div>
        ...
        <div className="p-4 sticky bottom-0 bg-[#f8f6f2]/95 backdrop-blur border-t">
          <div className="max-w-[700px] mx-auto bg-white border-2 border-black rounded-full p-1.5 flex gap-2 shadow-xl">
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Ask vault / chat with agents..." className="flex-1 bg-transparent px-6 outline-none text-[15px] font-medium text-black" />
