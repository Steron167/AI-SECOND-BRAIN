...
export default function Page() {
  ...
  async function trackResearch() {
    ...
    try {
      const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: `Research with FULL ReAct: ${topic} vs ${competitor || "general"} | Format Thought:/Action:/Observation:/Final Answer:`, memory }) })
    ...
    return blocks.map((block, idx) => {
      ...
      if (l.includes("action:")) return (
        <div key={idx} className="mb-5 rounded-2xl bg-[#eff6ff] border-2 border-blue-200 p-5">
          ...
          <div className="text-[14px] font-mono font-semibold leading-7 text-black bg-white p-3 rounded-xl border border-blue-100 whitespace-pre-wrap break-words">{block.replace(/Action:/i,"").trim()}</div>
        </div>
      )
      ...
      if (l.includes("final answer:")) {
        ...
        return (
          <div key={idx} className="rounded-2xl bg-black text-white p-6 border-2 border-black">
            <div className="flex gap-2 mb-4 items-center"><div className="w-7 h-7 rounded-full bg-white text-black flex items-center justify-center text-xs font-bold">✓</div><div className="text-[12px] font-extrabold tracking-widest">FINAL ANSWER • SYNTHESIZED</div><div className="ml-auto text-[10px] px-2.5 py-1 rounded-full bg-white text-black font-bold">Multi-Agent Output</div></div>
            <div className="space-y-3">
              {lines.map((line, li) => {
                if (line.trim().startsWith("|") && line.includes("|")) {
                  const cells = line.split("|").filter(c=>c.trim()).map(c=>c.trim())
                  if (cells.length===0 || cells[0].includes("---")) return null
                  return <div key={li} className="grid gap-2" style={{gridTemplateColumns:`repeat(${cells.length}, minmax(0,1fr))`}}>{cells.map((cell, ci)=><div key={ci} className={`px-3 py-2.5 rounded-xl text-[13px] font-bold border ${ci===0? "bg-zinc-800 text-zinc-200 border-zinc-700" : "bg-white text-black border-white"}`}>{cell}</div>)}</div>
                }
  ...
  const Node = ({ id, title, sub }: any) => (
    <div className={`rounded-[14px] p-3 border-2 flex items-center gap-3 transition-all ${activeNode===id? "bg-white text-black border-white shadow-[0_6px_24px_rgba(255,255,255,0.25)] scale-[1.02]" : "bg-[#1e1e1e] border-[#2c2c2c] text-white"}`}>
    ...
    <div className="min-h-screen bg-[#f8f6f2] flex font-[Inter] antialiased">
      ...
      <div className="w-[380px] hidden lg:flex flex-col bg-[#0b0b0b] text-white p-6 shrink-0 border-r border-zinc-900">
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col gap-1 items-end">
            <div className="text-[10px] bg-white text-black px-3 py-1 rounded-full font-extrabold">LangGraph</div>
            <div className="text-[9px] px-2.5 py-1 rounded-full bg-[#00ff66] text-black font-extrabold tracking-wide">6 AGENTS ACTIVE</div>
          </div>
        </div>
        <div className="text-[12px] text-zinc-300 mt-3 font-medium leading-5">Multi Agent Framework • Parallel Execution • Memory Graph</div>
        <div className="mt-2 text-[10px] text-zinc-500 font-bold tracking-wide">Why LangGraph {" > "} CrewAI? Stateful, checkpointing, parallel, lightweight</div>

        <div className="mt-7 space-y-2.5">
          <div className="text-[10px] tracking-[0.2em] text-zinc-500 font-extrabold mb-3 flex justify-between">
            <span>LANGGRAPH EXECUTION GRAPH</span>
            <span className="text-[#00ff66]">MULTI-AGENT</span>
          </div>
          ...
          <Node id="resolver" title="CONFLICT RESOLVER AGENT" sub="Evidence verification $74.5k" />
          ...
          <div className={`mt-3 p-3.5 rounded-xl text-center text-[12px] font-extrabold border-2 tracking-wide ${activeNode==="done"? "bg-[#00ff66] text-black border-[#00ff66] shadow-[0_0_20px_rgba(0,255,102,0.3)]" : "bg-[#171717] border-[#2a2a2a] text-zinc-500"}`}>
            {activeNode==="done"? "✓ GRAPH COMPLETE - ALL 6 AGENTS FINISHED" : "IDLE - WAITING FOR MULTI-AGENT TASK"}
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-xl p-3 text-center border-2 border-white"><div className="text-[9px] font-bold text-zinc-500 tracking-wide">CONFIDENCE</div><div className="text-[16px] font-extrabold text-black mt-1">{meta?.metrics?.confidence || 0.85}</div><div className="text-[9px] text-green-600 font-bold mt-1">High</div></div>
            <div className="bg-[#1c1c1c] rounded-xl p-3 text-center border-2 border-[#2c2c2c]"><div className="text-[9px] font-bold text-zinc-400 tracking-wide">RETRIES</div><div className="text-[16px] font-extrabold text-orange-400 mt-1">{meta?.metrics?.retries || 0}</div><div className="text-[9px] text-zinc-500 font-bold mt-1">Fallback</div></div>
            <div className="bg-[#1c1c1c] rounded-xl p-3 text-center border-2 border-[#2c2c2c]"><div className="text-[9px] font-bold text-zinc-400 tracking-wide">STEPS</div><div className="text-[16px] font-extrabold text-white mt-1">{checkpoints.length || 6}</div><div className="text-[9px] text-zinc-500 font-bold mt-1">Checkpoints</div></div>
          </div>
        </div>

        <div className="mt-6 bg-[#151515] rounded-xl border border-zinc-800 p-4">
          <div className="text-[10px] font-bold tracking-widest text-zinc-400">MULTI AI AGENT ROLES</div>
          <div className="mt-2 space-y-1.5 text-[11px] font-medium text-zinc-300">
            <div>• Planner → breaks task</div>
            <div>• Recaller → memory recall</div>
            <div>• Researcher x2 → parallel search</div>
            <div>• Resolver → conflict fix</div>
            <div>• Evaluator → quality check</div>
            <div>• Librarian → persist</div>
          </div>
        </div>

        <div className="mt-auto pt-6">
          <div className="text-[10px] text-zinc-600 leading-4 font-medium">Multi AI Agent System: Each agent is a specialized LLM node in StateGraph. Executes in parallel, recovers from 30% tool failure via fallback, verifies conflicts.</div>
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="p-6 md:p-8 max-w-[900px] mx-auto w-full">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-[28px] font-extrabold text-black tracking-tight leading-none">Research Intelligence</h1>
              <p className="text-[14px] font-medium text-zinc-600 mt-2 flex items-center gap-2"><span className="px-2.5 py-1 rounded-full bg-black text-white text-[11px] font-bold">MULTI AI AGENT</span> 6 agents • Parallel • ReAct trace • Full length</p>
            </div>
          </div>
          ...
          <div className="mt-6 bg-white rounded-[24px] border-2 border-black/10 shadow-xl p-1.5">
            <div className="bg-[#f6f4f1] rounded-[18px] p-5">
              <div className="flex items-center gap-2 mb-4"><div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center font-bold">⌕</div><div className="text-[14px] font-extrabold text-black">Search → Multi AI Agent Pipeline</div><div className="ml-auto text-[10px] px-2.5 py-1 rounded-full bg-[#00ff66] text-black font-bold">6 Agents</div></div>
              <div className="grid md:grid-cols-[1.3fr_1fr] gap-3">
                <div><label className="text-[11px] font-bold text-zinc-500 ml-1">TOPIC</label><input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="Spider-Man vs Batman" className="w-full mt-1 bg-white rounded-full px-5 py-3.5 text-[15px] font-semibold text-black border-2 border-zinc-200 focus:border-black outline-none" /></div>
                <div><label className="text-[11px] font-bold text-zinc-500 ml-1">VS COMPETITOR</label><input value={competitor} onChange={e=>setCompetitor(e.target.value)} placeholder="Compare" className="w-full mt-1 bg-white rounded-full px-5 py-3.5 text-[15px] font-semibold text-black border-2 border-zinc-200 focus:border-black outline-none" /></div>
              </div>
          ...
          {result && (
            <div className="mt-6 bg-white rounded-[24px] border-2 border-black shadow-xl overflow-hidden">
              <div className="px-6 py-4 bg-black text-white flex justify-between items-center">
                <div className="text-[11px] font-extrabold tracking-widest">FINAL ANSWER • MULTI AI AGENT • VERIFIED • FULL LENGTH</div>
                <div className="flex gap-2"><span className="text-[11px] px-3 py-1 rounded-full bg-white text-black font-bold">{result.length} chars • Full</span><span className="text-[11px] px-3 py-1 rounded-full bg-[#00ff66] text-black font-bold">6 Agents</span></div>
              </div>
        ...
        <div className="p-4 sticky bottom-0 bg-[#f8f6f2]/95 backdrop-blur border-t">
          <div className="max-w-[700px] mx-auto bg-white border-2 border-black rounded-full p-1.5 flex gap-2 shadow-xl">
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Ask vault: what did multi agents research?" className="flex-1 bg-transparent px-6 outline-none text-[15px] font-medium text-black" />
